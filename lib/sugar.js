...
function addAll(arr) {
  let keys1 =[];
  let values2=[];
  //go into each element in the array
  for (let i = 0; i < arr.length; i++) {
    //split the object into an array of values and keys
    keys1.push(Object.keys(arr[i])); //push into the new keys array
    values2.push(Object.values(arr[i])); //push into the new value array
    
  result2 = Object.assign(...keys1.map((k, x) => ({[k]:values2[x]})))
  }
  return result2; //then make a new object using assign to put all the keys and values together

}

    //then make a new object using assign to put all the keys and values together
